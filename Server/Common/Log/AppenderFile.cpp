...
{
	if (extraArgs.empty())
		throw InvalidAppenderArgsException(Tornado::StringFormat("Log::CreateAppenderFromConfig: Missing file name for appender %s\n", name.c_str()));
...
FILE* AppenderFile::OpenFile(std::string const& filename, std::string const& mode, bool backup)
{
	...
	FILE* ret = nullptr;
	auto error = fopen_s(&ret, fullName.c_str(), mode.c_str());
	if (nullptr != ret)
	{
		_fileSize = ftell(ret);
		return ret;
